...
- (void)viewDidLoad {
    ...
    [plannerItems addObject:@"1-Year Bible Study Planner"];
    [plannerItems addObject:@"2-Year Bible Study Planner"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.row == 0){
		ROR_BibleAppDelegate *appDelegateForOneYear = 
		[[UIApplication sharedApplication] delegate];
		appDelegateForOneYear.viewController.oneYear = [plannerItems objectAtIndex:indexPath.row];
		
	}
	
	if (indexPath.row == 1){
		ROR_BibleAppDelegate *appDelegateForTwoYear = 
		[[UIApplication sharedApplication] delegate];
		appDelegateForTwoYear.viewController.twoYears = [plannerItems objectAtIndex:indexPath.row];
